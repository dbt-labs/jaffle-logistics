-- Classify each incident-report chunk's content type (REAL cloud
...
{{ config(
    materialized='table',
    pre_hook=[
        "{{ dbt_context_engineering.guard_batch(ref('incident_reports_hashed'), 'chunk_text') }}",
        "{{ dbt_context_engineering.log_ai_run('classify', model_name='ai_classify (built-in)', relation=ref('incident_reports_hashed'), input_column='chunk_text') }}"
    ],
