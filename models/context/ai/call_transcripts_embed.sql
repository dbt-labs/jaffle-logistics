-- Governed incremental embedding of call-transcript chunks (REAL cloud
-- AI-function cost). DO NOT run until spend is approved. Same governed
-- pattern as legal_docs_embed.sql.
--
...
{{ config(
    ...
    unique_key='chunk_id',
    pre_hook=[
        "{{ dbt_context_engineering.guard_batch(ref('call_transcripts_hashed'), 'chunk_text', filter=dbt_context_engineering.incremental_delta_predicate('chunk_id', dbt_context_engineering.embedding_fn_fingerprint(model=var('embedding_model')), 'embedding_fn_fingerprint', content_hash_column='content_hash')) }}",
        "{{ dbt_context_engineering.log_ai_run('embed', model_name=var('embedding_model'), relation=ref('call_transcripts_hashed'), input_column='chunk_text', filter=dbt_context_engineering.incremental_delta_predicate('chunk_id', dbt_context_engineering.embedding_fn_fingerprint(model=var('embedding_model')), 'embedding_fn_fingerprint', content_hash_column='content_hash')) }}"
    ],
...
with embedded as (
    select
        ...
        {%- endif %} as embedding
    from {{ ref('call_transcripts_hashed') }}
