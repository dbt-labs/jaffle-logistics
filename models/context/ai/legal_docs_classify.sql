-- Classify each legal-doc chunk's content type (REAL cloud AI-function
...
{{ config(
    materialized='table',
    pre_hook=[
        "{{ dbt_context_engineering.guard_batch(ref('legal_docs_hashed'), 'chunk_text') }}",
        "{{ dbt_context_engineering.log_ai_run('classify', model_name='ai_classify (built-in)', relation=ref('legal_docs_hashed'), input_column='chunk_text') }}"
    ],
