-- Score CRM-sentiment predictions vs. ground truth (accuracy + per-label
-- precision/recall). Deterministic, zero AI cost. Thresholded by a test in
-- _context_ai.yml.


with _base as (
    select
        predicted_sentiment as pred,
        true_sentiment   as expected
    from sthibeault_test_db.dbt_sthibeault.crm_sentiment_scored
),
_overall as (
    select
        'accuracy' as metric,
...
select
    'meridian_crm_sentiment/v1' as prompt_version,
    metric,
    label,
    value
from _metrics